...
$(".puzzle-image").css(
  "margin",
  "-" + imgHeight / 2 + "px 0 0 -" + imgWidth / 2 + "px"
);
...
function Html5Puzzle(config) {
  ...
  function createTiles(xTileCount, yTileCount) {
    ...
    for (var y = 0; y < yTileCount; y++) {
      for (var x = 0; x < xTileCount; x++) {
        ...
        img.visible = true;
        console.log("createTiles 22222222 " + y + " , " + x);
        console.log(img);
    ...
    for (var y = 0; y < yTileCount; y++) {
      for (var x = 0; x < xTileCount; x++) {
        ...
        var position =
          ...
          new Point(
            instance.tileWidth * (x * 2 + (y % 2)),
            instance.tileWidth * y
          ) -
          new Point(
            instance.puzzleImage.size.width,
            instance.puzzleImage.size.height / 2
          );
  ...
  this.pickTile = function () {
    if (instance.selectedTile) {
      ...
      var pos = new Point(
        instance.selectedTile.position.x,
  ...
  this.releaseTile = function () {
    if (instance.selectedTile) {
      ...
      if (topTile) {
        hasConflict =
          hasConflict ||
          !(topTile.shape.bottomTab + instance.selectedTile.shape.topTab == 0);
      }
      ...
      if (bottomTile) {
        hasConflict =
          hasConflict ||
          !(
            bottomTile.shape.topTab + instance.selectedTile.shape.bottomTab ==
            0
          );
      }
      ...
      if (rightTile) {
        hasConflict =
          hasConflict ||
          !(
            rightTile.shape.leftTab + instance.selectedTile.shape.rightTab ==
            0
          );
      }
      ...
      if (leftTile) {
        hasConflict =
          hasConflict ||
          !(leftTile.shape.rightTab + instance.selectedTile.shape.leftTab == 0);
      }
  ...
  this.mouseMove = function (point, delta) {
    if (!instance.selectionGroup) {
      ...
      if (delta.x < 8 && delta.y < 8) {
        var tolerance = instance.tileWidth * 0.5;
        var hit = false;
        for (var index = 0; index < instance.tiles.length; index++) {
          ...
          hit =
            deltaPoint.x * deltaPoint.x + deltaPoint.y * deltaPoint.y <
            tolerance * tolerance;
        ...
        if (!hit) instance.selectedTile = null;
      }
    } else {
